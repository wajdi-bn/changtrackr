...
$assignmentCount = & $az role assignment list --assignee $servicePrincipalId --scope $vmId --role 'Virtual Machine Contributor' --query 'length(@)' -o tsv
if ($assignmentCount -eq '0') {
    & $az role assignment create --assignee-object-id $servicePrincipalId --assignee-principal-type ServicePrincipal --role 'Virtual Machine Contributor' --scope $vmId --only-show-errors | Out-Null
    if ($LASTEXITCODE -ne 0) { throw 'Could not grant the VM deployment role.' }
}
