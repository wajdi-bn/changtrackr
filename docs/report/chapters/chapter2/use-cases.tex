...
Le diagramme global de la figure~\ref{fig:global-use-cases} reste volontairement
macroscopique. Il présente les objectifs majeurs et les systèmes externes sans
transformer l'authentification en une forêt de relations \texttt{include}.
L'authentification est une précondition des cas protégés ; les rôles sont des
spécialisations de l'utilisateur authentifié décrites dans le
tableau~\ref{tab:human-actors}.

\begin{landscape}
  \begin{figure}[p]
    \centering
    \resizebox{0.98\linewidth}{!}{\input{assets/diagrams/global-use-cases}}
...
\begin{scopebox}[Conventions du diagramme]
  Les rectangles verts représentent les acteurs humains ; les rectangles bleus,
  les systèmes externes ; les ellipses, les objectifs fournis par la plateforme.
  Les relations en pointillés indiquent une dépendance fonctionnelle de haut niveau
  et non une séquence complète. Les contrôles de permission, l'organisation et les
  scénarios d'erreur sont détaillés ci-dessous.
\end{scopebox}
