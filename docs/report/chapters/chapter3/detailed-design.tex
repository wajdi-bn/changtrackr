...
Le diagramme de séquence de la figure~\ref{fig:uc01-sequence} précise les échanges
entre le visiteur, le super administrateur, l'API, la base et le service email.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|p{3.5cm}|Y|Y|}
    \hline
    \rowcolor{ctmint}
    \textbf{Cible} & \textbf{Comportement} & \textbf{Usage}\\\hline
...
\begin{figure}[H]
  \centering
  \resizebox{0.98\textwidth}{!}{\input{assets/diagrams/availability-pipeline}}
...
\begin{longtable}{|p{1.0cm}|p{5.2cm}|p{3.5cm}|p{4.4cm}|}
  ...
  Recharge interdite jusqu'à levée de l'override.\\\hline
  2 & Maintenance active & \texttt{maintenance} &
  Indisponibilité planifiée et commande \texttt{ChangeAvailability}.\\\hline
  3 & Déconnexion ou silence supérieur à 90 secondes & \texttt{offline} &
  Connecteurs hors ligne et alerte de communication.\\\hline
  4 & Au moins un connecteur \texttt{Available} & \texttt{available} &
  Démarrage distant autorisé si l'organisation est active.\\\hline
  5 & Aucun libre, au moins un en transaction & \texttt{charging} &
  Station vivante mais sans place libre correspondante.\\\hline
  6 & Aucun libre, au moins un réservé & \texttt{reserved} &
  Réservation visible ; recharge publique non immédiate.\\\hline
  7 & Aucun utilisable, au moins un \texttt{Faulted} & \texttt{faulted} &
  Alerte de défaut dédupliquée.\\\hline
...
La figure~\ref{fig:uc05-sequence} présente la séquence système complète, depuis la
sélection du connecteur jusqu'à la capture du paiement final.
...
\begin{figure}[H]
  \centering
  \resizebox{0.96\textwidth}{!}{\input{assets/diagrams/operations-lifecycle}}
