\section{Conception fonctionnelle et scénarios d'utilisation}

Cette section complète la spécification par une conception orientée scénarios.
Elle part du diagramme global, identifie les cas majeurs, puis détaille pour chacun
le contrat fonctionnel, le déroulement nominal et les exceptions. Chaque fiche est
suivie d'un emplacement titré pour son diagramme de cas d'utilisation raffiné et
d'un diagramme de séquence système construit à partir du scénario validé. Les
séquences décrivent uniquement des interactions présentes dans le Product Backlog
et dans la réalisation.

\newcommand{\UseCaseUmlPlate}[3]{%
  \begin{figure}[p]
    \centering
    \begin{tcolorbox}[
      width=0.94\textwidth,
      ...
      {\large\bfseries\color{ctdark}Diagramme de cas d'utilisation raffiné}\\[2mm]
      {\bfseries #1}\\[2mm]
      {\color{ctgray}Emplacement réservé pour #1}\\[1mm]
      {\small Acteurs, frontière du système, spécialisations et relations
      \texttt{include}/\texttt{extend}.}
    \end{tcolorbox}
    \caption{Emplacement du diagramme de cas d'utilisation raffiné -- #1}
    \label{fig:#2-refined-use-case}
  \end{figure}

  \begin{figure}[p]
    \centering
    \resizebox{0.96\textwidth}{!}{\input{#3}}
    \caption{Diagramme de séquence système -- #1}
    \label{fig:#2-sequence}
  \end{figure}
  \clearpage
}
...
\begin{figure}[p]
  ...
  \caption{Diagramme de cas d'utilisation global de \ProjectName{}}
...
\begin{longtable}{|C{1.35cm}|p{4.0cm}|p{3.3cm}|p{5.3cm}|}
  ...
  être reliés au simulateur ou au matériel.\\\hline
  ...
  L'incident est qualifié, assigné, documenté et clôturé avec une chronologie.\\\hline
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|C{3.3cm}|Y|}
    ...
    d'essai et compte administrateur sécurisé.\\\hline
    \rowcolor{ctmint}
    ...
    n'est pas bloquée par les protections anti-abus.\\\hline
    \rowcolor{ctmint}
    \textbf{Déclencheur} & Le visiteur valide le formulaire public.\\\hline
    \rowcolor{ctmint}
    \textbf{Postconditions} & La demande est rejetée ou reliée à une organisation
    d'essai et à une invitation administrateur traçable.\\\hline
...
\UseCaseUmlPlate
  ...
  {uc01}
  {assets/diagrams/account-provisioning-sequence}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|C{3.3cm}|Y|}
    ...
    mot de passe initial.\\\hline
    \rowcolor{ctmint}
    \textbf{Acteurs} & Administrateur, employé invité et service email.\\\hline
    \rowcolor{ctmint}
    \textbf{Préconditions} & L'administrateur et son organisation sont actifs ; la
    limite d'employés de l'offre commerciale n'est pas atteinte.\\\hline
    \rowcolor{ctmint}
    \textbf{Déclencheur} & L'administrateur choisit \emph{Ajouter un employé}.\\\hline
    \rowcolor{ctmint}
    \textbf{Postconditions} & Le compte activé appartient à exactement une
    organisation et possède uniquement le rôle choisi.\\\hline
...
\UseCaseUmlPlate
  ...
  {uc02}
  {assets/diagrams/employee-invitation-sequence}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|C{3.3cm}|Y|}
    ...
    son identité de communication dans une seule opération.\\\hline
    \rowcolor{ctmint}
    \textbf{Acteurs} & Opérateur ou Administrateur ; borne physique ou simulateur SAP.\\\hline
    \rowcolor{ctmint}
    \textbf{Préconditions} & L'organisation est autorisée à ajouter une station et
    n'a pas atteint sa capacité commerciale.\\\hline
    \rowcolor{ctmint}
    \textbf{Déclencheur} & L'utilisateur ouvre l'assistant de mise en service.\\\hline
    \rowcolor{ctmint}
    \textbf{Postconditions} & La station est persistée avec ses connecteurs et une
    cible de commissioning ; son secret en clair n'est affiché qu'une fois si requis.\\\hline
...
\UseCaseUmlPlate
  ...
  {uc03}
  {assets/diagrams/station-commissioning-sequence}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|C{3.3cm}|Y|}
    ...
    état métier explicable.\\\hline
    \rowcolor{ctmint}
    \textbf{Acteurs} & Borne ou simulateur ; moteur de projection de la plateforme.\\\hline
    \rowcolor{ctmint}
...
\UseCaseUmlPlate
  ...
  {uc04}
  {assets/diagrams/availability-projection-sequence}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|C{3.3cm}|Y|}
    ...
    régler le montant final sans créer de session fictive.\\\hline
    \rowcolor{ctmint}
    \textbf{Acteurs} & Client, borne OCPP et service de paiement.\\\hline
    \rowcolor{ctmint}
    \textbf{Préconditions} & Client actif et vérifié, station publique connectée,
    connecteur disponible, tarif effectif et aucune session concurrente.\\\hline
    \rowcolor{ctmint}
    \textbf{Déclencheur} & Le client choisit \emph{Recharger} sur une station.\\\hline
    \rowcolor{ctmint}
    \textbf{Postconditions} & La transaction est arrêtée, le montant final est
    capturé ou signalé impayé, et un reçu consultable est associé.\\\hline
...
\UseCaseUmlPlate
  ...
  {uc05}
  {assets/diagrams/charging-payment-sequence}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|C{3.3cm}|Y|}
    ...
    jusqu'à la résolution.\\\hline
    \rowcolor{ctmint}
    \textbf{Acteurs} & Opérateur et Technicien ; service email pour les événements
    critiques ou assignations.\\\hline
    \rowcolor{ctmint}
    \textbf{Préconditions} & L'alerte appartient à l'organisation et l'opérateur
    dispose du droit de gestion.\\\hline
    \rowcolor{ctmint}
    \textbf{Déclencheur} & Création automatique d'une alerte ou signalement autorisé.\\\hline
    \rowcolor{ctmint}
    \textbf{Postconditions} & L'alerte est résolue ou documentée pour suivi ; le
    rapport et les preuves restent accessibles aux acteurs autorisés.\\\hline
...
\UseCaseUmlPlate
  ...
  {uc06}
  {assets/diagrams/alert-intervention-sequence}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|C{3.3cm}|Y|}
    ...
    restent cohérents et audités.\\\hline
...
\UseCaseUmlPlate
  ...
  {uc07}
  {assets/diagrams/organization-commercial-sequence}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|C{3.3cm}|Y|}
    ...
    rôle, export ou rapport interne traçable.\\\hline
    \rowcolor{ctmint}
    \textbf{Acteurs} & Super Administrateur, Administrateur, Opérateur ou Technicien.\\\hline
    \rowcolor{ctmint}
    \textbf{Préconditions} & L'utilisateur possède les permissions de consultation,
    d'export ou d'échange correspondantes.\\\hline
    \rowcolor{ctmint}
    \textbf{Déclencheur} & Sélection d'une période ou création d'un brouillon.\\\hline
    \rowcolor{ctmint}
    \textbf{Postconditions} & L'export est téléchargé ou le rapport est envoyé à un
    destinataire autorisé avec son historique.\\\hline
...
\UseCaseUmlPlate
  ...
  {uc08}
  {assets/diagrams/report-exchange-sequence}
