...
Cette structure a facilité le diagnostic. Un écran n'actualisant pas une donnée
...
émis par le simulateur. Le fournisseur de paiement peut de la même manière être
observé sans modifier les tables depuis WireMock.

\subsection{Stratégie de tests}

La stratégie accorde la priorité aux invariants backend, car le frontend ne doit
pas être la seule barrière contre une action interdite. Les tests utilisent une
base SQLite en mémoire, une queue synchrone, un mailer en mémoire et un diffuseur
nul. Ils restent ainsi reproductibles et n'envoient ni email réel ni événement
WebSocket pendant la campagne.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{4.5cm}p{3.2cm}Y@{}}
    \toprule
    \textbf{Contrôle} & \textbf{Résultat} & \textbf{Portée}\\
    \midrule
    \texttt{php artisan test --compact} &
    201 tests, 1\,489 assertions & 37 classes de tests Feature et tests unitaires
    chargés par PHPUnit.\\
    \texttt{pnpm test:ocpp-tools} &
    2 tests, 0 échec & Lecture et contrôle des états du simulateur.\\
    \texttt{pnpm lint:frontend} &
    Succès & Analyse Oxlint du code TypeScript/React.\\
    \texttt{pnpm build:frontend} &
    Succès & Type-check TypeScript et bundle Vite de production.\\
    Inventaire des routes &
    186 routes, dont 177 API & 75 GET, 67 POST, 23 PATCH/PUT et 12 DELETE.\\
    \bottomrule
  \end{tabularx}
\end{table}
...
après l'intégration des incréments.
...
lents, notamment emails et certaines notifications, passent par la queue.
...
La compilation frontend signale toutefois plusieurs bundles supérieurs à
500~kio ; le principal fichier JavaScript atteint environ 767~kio. Ce résultat
n'empêche pas le MVP local, mais justifie une optimisation avant production :
découpage par route, chargement différé de PDF.js et du scanner QR, séparation des
icônes et graphiques, puis mesure Lighthouse sur réseau mobile.

\subsection{Sécurité appliquée}

Les mesures suivantes sont effectivement présentes dans le code :

\begin{itemize}[leftmargin=7mm]
  \item session SPA protégée par Sanctum et cookie CSRF ;
  \item validation systématique des écritures par \emph{Form Requests} ;
  \item policies et scoping organisationnel côté serveur ;
  \item hachage des mots de passe et des secrets de station ;
  \item vérification des signatures de webhooks et déduplication des événements ;
  \item limitation des tentatives de connexion, d'activation et d'invitation ;
  \item routes internes OCPP séparées des routes publiques ;
  \item journal d'audit pour les actions sensibles ;
  \item fichiers et documents servis par contrôleurs autorisés ;
  \item secrets exclus du dépôt et masqués dans les réponses d'intégration.
\end{itemize}

Un audit de sécurité complet reste nécessaire avant exposition publique :
rotation des clés, TLS/WSS, durcissement des cookies, en-têtes de sécurité,
analyse des dépendances, tests IDOR systématiques, sauvegardes chiffrées et
supervision des événements anormaux.
...
\begin{itemize}[leftmargin=7mm]
  ...
  \item La spécification OpenAPI/Swagger, les manuels et le procès-verbal de recette
  constituent des livrables distincts encore à finaliser.
  \item OCPP 2.0.1, Microsoft OAuth, le paiement réel et le firmware restent hors
  du MVP validé.
\end{itemize}
\end{scopebox}

\subsection{Traçabilité avec le backlog}

\begin{longtable}{@{}C{1.2cm}p{3.4cm}p{4.6cm}p{4.1cm}@{}}
  \caption{Preuves de réalisation par incrément}
  \label{tab:implementation-traceability}\\
  \toprule
  \textbf{Inc.} & \textbf{User stories} & \textbf{Preuves dans le code} &
  ...
  \midrule
  \endhead
  INC-01 & US-01--04, 06, 27, 39 &
  Auth, Google, invitations, démo, onboarding, policies &
  Auth, OAuth, onboarding, utilisateurs, organisations.\\
  INC-02 & US-07--13, 37 &
  Stations, connecteurs, cartes, commissioning, documents &
  Station API, commissioning, documents.\\
  INC-03 & US-14--16, 19 &
  Gateway, ingestion, projection, commandes, télémétrie &
  OCPP gateway, flotte, supervision, disponibilité, télémétrie.\\
  INC-04 & US-20--23, 40 &
  Alertes, interventions, maintenance, SLA, notifications &
  Alertes, rapports terrain, maintenance, notifications.\\
  INC-05 & US-24--26, 28--30 &
  Tentatives, sessions, transactions, paiements, reçus &
  Recharge distante, session/paiement, webhook WireMock.\\
  INC-06 & US-31, 32, 41, 42 &
  Tarifs, plans, abonnements clients et SaaS &
  Tarifs, plans, commercial organisationnel.\\
  INC-07 & US-05, 17, 18, 33--36 &
  Dashboards, reporting, audit, paramètres, intégrations &
  Dashboard, rapports, gouvernance, recherche.\\
  INC-08 & US-34, 38 &
  Exports PDF/JSON/CSV ; firmware documenté mais différé &
  Exports couverts dans les tests de chaque domaine.\\
  \bottomrule
\end{longtable}

\section{Documentation technique produite et restante}

Le dépôt contient déjà les scripts d'installation, les configurations d'exemple,
les commandes d'exploitation des simulateurs et le présent rapport. Les noms des
routes et les ressources Laravel forment une base vérifiable pour produire une
spécification OpenAPI 3.1. Celle-ci sera exposée avec Swagger UI pour l'exploration,
mais le fichier OpenAPI versionné restera la source du livrable API.

Les livrables à finaliser sont le manuel utilisateur par rôle, le manuel
administrateur, le cahier de tests, la procédure d'installation, la documentation
des processus OCPP et le procès-verbal de recette. Ils devront reprendre les mêmes
identifiants de user stories et critères d'acceptation afin d'éviter une
documentation déconnectée du produit.
