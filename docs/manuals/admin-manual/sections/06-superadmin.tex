...
La capture ci-dessous provient de l'environnement local et affiche le relais
SMTP de développement. Le déploiement de référence utilise Resend avec le
domaine d'envoi vérifié \texttt{chargetrackr.me}.

\manualscreenshotcompact{superadmin-integrations.png}
  {État opérationnel local des intégrations de la plateforme}
  {fig:superadmin-integrations}
...
\begin{procedurebox}{Analyser une trace d'audit}
  \begin{enumerate}[leftmargin=*]
    \item Définir une période courte, puis filtrer par action, auteur ou
    organisation.
    \item Ouvrir l'événement et vérifier son horodatage, son entité et sa
    référence métier.
    \item Corréler la trace avec l'écran concerné et, si nécessaire, avec les
    journaux techniques du serveur.
    \item Documenter la conclusion sans recopier de secret ni de donnée
    personnelle inutile.
    \item Exporter uniquement le sous-ensemble indispensable à l'analyse et le
    conserver selon la politique de rétention.
  \end{enumerate}
\end{procedurebox}
