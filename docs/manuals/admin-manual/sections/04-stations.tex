...
\begin{procedurebox}{Créer une station reliée au simulateur local}
  \begin{enumerate}[leftmargin=*]
    ...
    la carte.
    \item Décrire le matériel et ajouter chaque connecteur physique avec son
    libellé, son identifiant OCPP, son type de prise, son courant et sa
    puissance.
    \item Sélectionner la cible \emph{Local SAP simulator}. L'identité OCPP doit
    être unique et correspondre exactement à celle envoyée dans l'URL
    WebSocket.
    \item Vérifier le récapitulatif puis créer la station.
    \item Exécuter, depuis la racine du dépôt, la commande d'enregistrement
    affichée par l'interface.
    \item Redémarrer la pile OCPP et ouvrir le détail de la station.
  \end{enumerate}
\end{procedurebox}

\begin{verbatim}
npm run ocpp:add-simulator-station -- CT-TUN-010
npm run ocpp:down
npm run ocpp:up
\end{verbatim}
