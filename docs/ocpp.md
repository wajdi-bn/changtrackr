...
HMAC provides integrity and service authentication, not encryption. Production traffic between the
gateway and Laravel must therefore use HTTPS or a private encrypted service network. Charging
stations must use `wss://` outside the local simulator environment.
