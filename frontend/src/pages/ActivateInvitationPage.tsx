...
export function ActivateInvitationPage() {
  ...
  return <AuthPageShell eyebrow="Secure account invitation" title="Activate your ChargeTrackr account." description="Choose your password to access the organization and role assigned to you.">
    ...
    {invitationQuery.isLoading ? <div className="prototype-auth-state"><Spin size="large" /></div> : invalid ? <div className="prototype-auth-state"><Alert type="error" showIcon title="Invitation unavailable" description="The link is incomplete, expired, cancelled, or has already been used." /><Link className="prototype-auth-primary-link" to="/">Return to ChargeTrackr</Link></div> : isComplete ? <div className="prototype-auth-state"><Alert type="success" showIcon title="Account activated" description="Your password is set and access to your assigned workspace is ready." /><Link className="prototype-auth-primary-link" to="/login">Continue to sign in</Link></div> : <Form<ActivationValues> className="prototype-login-form" layout="vertical" requiredMark={false} onFinish={handleSubmit}>
