...
import { cancelEmployeeInvitation, deactivateManagedUser, exportManagedUsers, getPlatformUsers, remindEmployeeInvitation, renewEmployeeInvitation, updateManagedUser } from '../features/users/userApi'
import { httpClient } from '../api/httpClient'
import type { ManagedUser, ManagedUserFilterStatus, ManagedUserPayload, PlatformUser } from '../types/user'
import type { UserRole } from '../types/auth'

interface OrganizationOption { id: number; name: string; status: string }
const roleOptions: Array<{ value: UserRole; label: string }> = [{ value: 'super_admin', label: 'Super Administrator' }, { value: 'admin', label: 'Organization Administrator' }, { value: 'operator', label: 'Operator' }, { value: 'technician', label: 'Technician' }, { value: 'client', label: 'Client' }]
...
export function PlatformUsersPage() {
  ...
  const usersQuery = useQuery({ queryKey: ['platform-users', filters], queryFn: () => getPlatformUsers(filters) })
  ...
  const columns: ColumnsType<PlatformUser> = [
    ...
    { title: 'Organization', key: 'organization', render: (_, user) => <div className="admin-stack-cell"><span>{accountScope(user)}</span><small>{user.roles[0] === 'client' ? 'Independent driver account' : user.team ?? 'No team assigned'}</small></div> },
    ...
    { title: '', key: 'actions', width: 108, align: 'right', render: (_, user) => <Space size={3} className="admin-row-actions"><Tooltip title="View details"><Button aria-label={`View ${user.name}`} type="text" icon={<Eye size={15} />} onClick={() => setSelectedUser(user)} /></Tooltip>{user.roles[0] !== 'client' && <Tooltip title="Edit account"><Button aria-label={`Edit ${user.name}`} type="text" icon={<PencilLine size={15} />} onClick={() => setEditor(user as ManagedUser)} /></Tooltip>}{user.roles[0] !== 'client' && user.id !== currentUser?.id && user.status === 'active' && <Popconfirm title="Suspend this account?" description="Active sessions will be revoked immediately." okText="Suspend" okButtonProps={{ danger: true }} onConfirm={() => deactivateMutation.mutate(user.id)}><Tooltip title="Suspend account"><Button aria-label={`Suspend ${user.name}`} type="text" danger icon={<UserRoundX size={15} />} /></Tooltip></Popconfirm>}</Space> },
  ]

  return <div className="super-admin-page platform-users-page">
    <MountainBanner color="purple" breadcrumb={['Super Admin', 'Users']} title="Platform users" count={summary?.total ?? 0} subtitle="Supervise every platform account, role, organization assignment and access state from one directory." />
    <AdminMetricGrid><AdminMetric icon={Users} label="All users" value={summary?.total ?? 0} helper="Employees and clients" /><AdminMetric icon={UserCheck} label="Active" value={summary?.active ?? 0} helper="Accounts with current access" tone="blue" /><AdminMetric icon={ShieldCheck} label="Administrators" value={(summary?.by_role.super_admin ?? 0) + (summary?.by_role.admin ?? 0)} helper="Platform and organization governance" tone="purple" /><AdminMetric icon={UserRoundX} label="Clients" value={summary?.by_role.client ?? 0} helper="Independent driver accounts" tone="orange" /></AdminMetricGrid>
    <AdminDataPanel title="All platform accounts" subtitle="Search employees and clients together, then narrow the directory by role, organization or status." extra={<ExportDropdown loading={exportMutation.isPending} onExport={(format) => exportMutation.mutate(format)} />}>
    ...
    <PlatformUserDrawer user={selectedUser} currentUserId={currentUser?.id} invitationLoading={invitationMutation.isPending} onClose={() => setSelectedUser(null)} onEdit={(user) => setEditor(user as ManagedUser)} onInvitation={(id, action) => invitationMutation.mutate({ id, action })} />
...
function PlatformUserDrawer({ user, currentUserId, invitationLoading, onClose, onEdit, onInvitation }: { user: PlatformUser | null; currentUserId?: number; invitationLoading: boolean; onClose: () => void; onEdit: (user: PlatformUser) => void; onInvitation: (id: number, action: 'remind' | 'renew' | 'cancel') => void }) {
  const client = user?.roles[0] === 'client'
  return <Drawer className="admin-detail-drawer" size={570} open={Boolean(user)} onClose={onClose} title="User account details" extra={user && !client ? <Button icon={<PencilLine size={14} />} onClick={() => onEdit(user)}>Edit</Button> : null}>{user && <div className="platform-user-detail"><header><Avatar size={58} src={user.avatar_url ?? undefined}>{initials(user.name)}</Avatar><div><h2>{user.name}</h2><p>{user.email}</p><AdminStatus status={lifecycleStatus(user)} /></div></header><section className="platform-user-facts"><div><small>Role</small><strong>{roleLabel(user.roles[0])}</strong></div><div><small>Organization</small><strong>{accountScope(user)}</strong></div><div><small>{client ? 'Account scope' : 'Team'}</small><strong>{client ? 'Platform-wide driver' : user.team ?? 'Not assigned'}</strong></div><div><small>Last login</small><strong>{formatLastLogin(user.last_login_at)}</strong></div></section><section className="platform-user-activity"><h3>{client ? 'Driver activity' : 'Account activity'}</h3><div>{client ? <><span><strong>{user.activity.charging_sessions}</strong><small>Charging sessions</small></span><span><strong>{user.activity.payments}</strong><small>Payments</small></span><span><strong>{formatDate(user.created_at)}</strong><small>Registered</small></span></> : <><span><strong>{user.activity.assigned_alerts}</strong><small>Assigned alerts</small></span><span><strong>{user.activity.assigned_interventions}</strong><small>Interventions</small></span><span><strong>{user.activity.charging_sessions}</strong><small>Sessions</small></span></>}</div></section>{client && <p className="platform-self-note">Client accounts are global and remain independent from organizations. Their network relationships are created through charging sessions and memberships.</p>}{user.invitation && <section className="platform-invitation"><h3>Activation invitation</h3><div><span>Status</span><AdminStatus status={user.invitation.status} /></div><div><span>Last sent</span><strong>{formatDate(user.invitation.last_sent_at)}</strong></div><div><span>Expires</span><strong>{formatDate(user.invitation.expires_at)}</strong></div>{user.roles.some((role) => role === 'operator' || role === 'technician') && <Space wrap><Button disabled={!user.invitation.can_remind} loading={invitationLoading} onClick={() => onInvitation(user.id, 'remind')}>Send reminder</Button><Button disabled={!user.invitation.can_renew} loading={invitationLoading} onClick={() => onInvitation(user.id, 'renew')}>Renew invitation</Button><Button danger disabled={!user.invitation.can_cancel} loading={invitationLoading} onClick={() => onInvitation(user.id, 'cancel')}>Cancel invitation</Button></Space>}</section>}{user.id === currentUserId && <p className="platform-self-note">This is your current account. Your own elevated role and active status cannot be removed.</p>}</div>}</Drawer>
}
...
function roleLabel(role?: UserRole): string { return roleOptions.find((option) => option.value === role)?.label ?? 'User' }
function accountScope(user: PlatformUser): string { return user.roles[0] === 'super_admin' ? 'Platform' : user.roles[0] === 'client' ? 'Independent' : user.organization?.name ?? 'Not assigned' }
function lifecycleStatus(user: PlatformUser): string { return user.status === 'pending' ? user.invitation?.status ?? 'pending' : user.status }
function initials(name: string): string { return name.split(' ').filter(Boolean).slice(0, 2).map((part) => part.charAt(0)).join('').toUpperCase() }
