...
import { Download, Eye, PencilLine, Search, ShieldCheck, UserCheck, UserRoundX, Users } from 'lucide-react'
...
export function PlatformUsersPage() {
  ...
  const exportMutation = useMutation({ mutationFn: () => exportManagedUsers(filters, 'csv'), onSuccess: (blob) => { const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'platform-users.csv'; anchor.click(); URL.revokeObjectURL(url) }, onError: () => void message.error('The platform user export could not be generated.') })
  ...
  return <div className="super-admin-page platform-users-page">
    ...
    <AdminDataPanel title="Administrators and users" subtitle="Filter global employee accounts and inspect their organization assignment." extra={<Button icon={<Download size={15} />} loading={exportMutation.isPending} onClick={() => exportMutation.mutate()}>Export CSV</Button>}>
