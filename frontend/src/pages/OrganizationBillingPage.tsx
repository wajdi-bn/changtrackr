...
import { OrganizationPlanCheckoutModal } from '../features/commercial/OrganizationPlanCheckoutModal'
import type { SimulatedPaymentFormValues } from '../features/payments/SimulatedPaymentFields'
import { toSimulatedPaymentSelection } from '../features/payments/simulatedPaymentModel'
...
export function OrganizationBillingPage() {
  ...
  const [checkoutInvoice, setCheckoutInvoice] = useState<OrganizationInvoice | null>(null)
  ...
  const requestMutation = useMutation({
    mutationFn: ({ plan, billingCycle, payment }: { plan: SaasPlan; billingCycle: BillingCycle; payment: SimulatedPaymentFormValues }) => {
      const selection = toSimulatedPaymentSelection(payment)
      return requestOrganizationPlan({ saas_plan_id: plan.id, billing_cycle: billingCycle, payment_method: selection.method, simulation_outcome: selection.simulation_outcome, idempotency_key: createIdempotencyKey() })
    },
    onSuccess: async (invoice) => { setCheckoutInvoice(invoice); await queryClient.invalidateQueries({ queryKey: ['organization-billing'] }); if (invoice.status === 'paid') void message.success('Payment completed and organization plan activated.') },
    onError: () => void message.error('The organization checkout could not be completed. Resolve any existing open invoice first.'),
  })
  ...
  const openInvoice = (invoice: OrganizationInvoice) => setInvoicePreview({ title: `Organization invoice ${invoice.number}`, filename: `organization-invoice-${invoice.number}.pdf`, load: () => downloadOrganizationInvoice(invoice.id) })
  const invoiceColumns: ColumnsType<OrganizationInvoice> = [
    ...
    { title: '', align: 'right', width: 110, render: (_, invoice) => <Button type="text" icon={<Eye size={15} />} onClick={() => openInvoice(invoice)}>View PDF</Button> },
  ]
  ...
  const openCheckout = (plan: SaasPlan) => {
  ...
  return <div className="page-stack organization-billing-page">
    ...
    <OrganizationPlanCheckoutModal
      ...
      onClose={() => { setCheckoutPlan(null); setCheckoutInvoice(null) }}
      onConfirm={(payment) => checkoutPlan && requestMutation.mutate({ plan: checkoutPlan, billingCycle: cycle, payment })}
